...
Status: created as `MYM-27` / `1b30919c-e851-4c18-a513-f42f4980fdf5`; Multica status is `done`, pending Codex audit.
...
Before accepting `MYM-27`, verify:

- The backend mock stream is generated from `api.rm_workbench.mock_data.load_pre_meeting_brief_fixture` and `api.rm_workbench.adapter.map_rm_skill_contract_to_agui_events`.
- The frontend default load path consumes the backend mock stream, with static transcript only as visible fallback.
- Product selection still resolves through `/api/rm-workbench/pending/resolve` and treats `resolved: 0` as an error.
- User-facing smoke UI and demo data remain Chinese-first.
- No real Hermes chat, CopilotKit runtime, real RM Skill, real customer/product data, or Memory writes were introduced.
